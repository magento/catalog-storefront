...
final class OptionMapper
{
     ...
     * @var string
     */
    private static $dtoClassName = OptionInterface::class;
    ...
    * @param mixed $value
    */
    private function setByKey(Option $dto, string $key, $value): void
    {
        switch ($key) {
            case "option_id":
            ...
            case "price_type":
                ...
                break;
            case "required":
                $dto->setRequired((string) $value);
                break;
            case "product_sku":
                $dto->setProductSku((string) $value);
                break;
            case "value":
                $convertedArray = [];
                foreach ($value as $element) {
                    $convertedArray[] = $this->objectManager
                        ->create(\Magento\CatalogStorefrontApi\Api\Data\OptionValueMapper::class)
                        ->setData($element)
                        ->build();
                }
                $dto->setValue($convertedArray);
                break;
        }
    }
}
