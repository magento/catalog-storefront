 ...
 */
final class ImportReviewsRequestMapper
{
    /**
     * @var string
     */
    private static $dtoClassName = ImportReviewsRequestInterface::class;
    ...
    * @param mixed $value
    */
    private function setByKey(ImportReviewsRequest $dto, string $key, $value): void
    {
        switch ($key) {
            case "reviews":
                $convertedArray = [];
                foreach ($value as $element) {
                    $convertedArray[] = $this->objectManager
                        ->create(\Magento\CatalogStorefrontApi\Api\Data\ImportReviewMapper::class)
                        ->setData($element)
                        ->build();
                }
                $dto->setReviews($convertedArray);
                break;
            case "store":
