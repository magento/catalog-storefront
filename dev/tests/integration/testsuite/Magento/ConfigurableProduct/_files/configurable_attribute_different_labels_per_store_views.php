...
$fixtureStore = $storeManager->getStore('fixturestore')->getId();
...
$attributeFrontendLabelFactory = $objectManager->get(AttributeFrontendLabelInterfaceFactory::class);
...
if (!$attribute->loadByCode($entityType, 'different_labels_attribute')->getAttributeId()) {
    $attribute->setData(
        [
            'frontend_label' => [
                0 => 'Different option labels dropdown attribute',
                $defaultInstalledStoreId => 'Default store label',
                $fixtureStore => 'Fixture store label'
            ],
